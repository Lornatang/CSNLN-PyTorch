...
__all__ = [
    "same_padding", "extract_image_patches", "reduce_sum", "ResidualConvBlock",
    "CrossScaleAttention", "NonLocalAttention", "MultiSourceProjection", "SelfExemplarMining",
    "CSNLN",
]
...
class CrossScaleAttention(nn.Module):
    def __init__(self, channels: int, reduction: int, kernel_size: int, stride: int, scale: int, softmax_scale: int = 10) -> None:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for conv1_weight_group, conv2_weight_group, assembly_weight_group in zip(conv1_weight_groups, conv2_weight_groups, assembly_weight_groups):
            # normalize
            conv2_weight_group = conv2_weight_group[0]  # [L, C, k, k]
            max_conv2_weight_group = torch.max(torch.sqrt(reduce_sum(torch.pow(conv2_weight_group, 2), axis=[1, 2, 3], keepdim=True)), self.escape)
            norm_conv2_weight_group = conv2_weight_group / max_conv2_weight_group

            # Compute correlation map [1*c*H*W]
            conv1_weight_group = same_padding(conv1_weight_group, [self.kernel_size, self.kernel_size], [1, 1], [1, 1])  # xi:
...
class MultiSourceProjection(nn.Module):
    def __init__(self, channels: int, kernel_size: list, scale: list) -> None:
        ...
        self.cross_scale_attention = CrossScaleAttention(channels=128, reduction=2, kernel_size=3, stride=1, scale=upscale_factor)
...
class SelfExemplarMining(nn.Module):
    def __init__(self, channels: int, kernel_size: list, scale: list):
        ...
        self.down_conv1 = nn.Sequential(
            nn.Conv2d(channels, channels, (stride_kernel_size, stride_kernel_size), (stride, stride), (padding, padding)),
            nn.PReLU(),
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.scale == 4:
            ...
            estimate = torch.add(down_conv3, diff_encode2)
...
class CSNLN(nn.Module):
    def __init__(self, upscale_factor: int):
        ...
        self.self_exemplar_mining = SelfExemplarMining(channels=128, kernel_size=3, scale=upscale_factor)
